...
class Five_Qubit_QECC:
	...
	def __init__(self):
		self.__qubits_physical = QuantumRegister(size=5, name='p')
		self.__qubits_for_checks = AncillaRegister(size=4, name='c')
		self.__syndromes = ClassicalRegister(4, name='s')
	...
	def get_logical_X(self):
		logical_X_qc = QuantumCircuit(self.__qubits_physical, name='5-Qubit Logical X')
		logical_X_qc.x(self.__qubits_physical[:])
	...
	def __get_error_checker(self):
		...
		checker_qc.h(self.__qubits_for_checks[:])
		for i in range(4): 
		# i=0: XZZXI -> i=1: IXZZX -> i=2: XIXZZ -> i=3: ZXIXZ
			checker_qc.cx(self.__qubits_for_checks[i], self.__qubits_physical[i%5])
			checker_qc.cz(self.__qubits_for_checks[i], self.__qubits_physical[(i+1)%5])
			checker_qc.cz(self.__qubits_for_checks[i], self.__qubits_physical[(i+2)%5])
			checker_qc.cx(self.__qubits_for_checks[i], self.__qubits_physical[(i+3)%5])
		checker_qc.h(self.__qubits_for_checks[:])
	...
	def get_error_corrector(self):
		...
		for j in range(5): # ... on qubit j
			...
			with corrector_qc.if_test((self.__syndromes, syndromes_int)):
				corrector_qc.x(self.__qubits_physical[j])
		...
		for j in range(5):
			...
			with corrector_qc.if_test((self.__syndromes, syndromes_int)):
				corrector_qc.z(self.__qubits_physical[j])
		...
		for j in range(5):
			...
			syndromes_int = syndromes_int % (2**4)
			with corrector_qc.if_test((self.__syndromes, syndromes_int)):
				corrector_qc.y(self.__qubits_physical[j])
